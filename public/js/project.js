...
if($('.stock_index').length >0 ) {
    $('td.edit_manufacturer').on('click',function(){
        if(mouse_is_inside == false) {
            $('input').remove();
            $('select').remove();
            $('td span').show();
            ...
            element = parent_element.find('span.value');
            project_id = parent_element.parent().attr('id');
            project_id = project_id.replace('stock_','');   // 37
        }
    });
}
...
$(document).on('click',function(event) {
    if( parent_element ) {
        if (! parent_element.is(event.target) && ! parent_element.has(event.target).length ) {
            ...
            if(name == 'name' || name == 'duration' || name == 'comment' || name == 'manager' || name == 'designer' || name == 'quantity' || name == 'manufacturer') {
                type = 'text';
            } else {
                type = 'date';
            }
            ...
            if(parent_element.find('input').length == 0) {
                if( parent_element.hasClass('select')) {
                    ...
                    if(parent_element.hasClass('edit_manufacturer')) {
                        $.each(manufacturers, function( id, manufacturer) {
                            option_designer+='<option value="'+manufacturer.id+'">'+manufacturer.name+'</option>' ;
                        });
                        parent_element.prepend('<select name="'+name+'" type="'+type+'" value="' + value + '" pattern="'+ pattern +'" title="' + title +'" required autofocus >'+option_designer+'</select><input name="id" type="hidden" value="' + project_id + '"/>');
                    }
    ...
    if(form_data) {
        $.ajax({
            ...
            success: function( response ) {
                if($('.stock_index').length >0 ) {
                    $('tr#stock_'+project_id).load(location.href + ' tr#stock_'+project_id +' td', function() {
