...
$(function() {
    function delete_employee() {
        $('.delete_employee').on('click',function(e){
            ...
            e.preventDefault();
            if (! confirm("Sigurno želiš obrisati djelatnika sa projekta?")) {
                return false;
            } else {
                url_delete = $( this ).attr('data-href');
    ...
    all_designins = JSON.parse($('.json_designins').text());
    console.log(all_designins);
    ...
    $.each(users, function( index_user, user ) {
        ...
        for (i = 0; i < 33; i++) {
            ...
            if (day_of_week >0 && day_of_week < 6 ) {
                ...
                append += '<td class="proj_user_color days '+ fst_day +'" id="' + user.id + '_' + today_day+'">';
                append += role == 'admin' ? '<a href="'+location.origin+'/designing_employees/create?user_id='+ user.id +'&start_date='+ today_day +'" rel="modal:open"><i class="fas fa-plus"></i></a>' : '';
                append += '</td>';
            }
...
{/* <span class="add_employee" ><i class="fas fa-plus"></i></span> */}
...
function timeline_drow () {    
    $( ".timeline tbody tr" ).each(function( index ) {
        ...
        user_designins = user_object.has_designing_employees;
        ...
        if(user_designins.length > 0) {
            $.each(user_designins, function( index_designin, designing ) {
                console.log(designing);
                project = $.grep( all_designins, function( n, i ) {
                    return n.id == designing.designing_id;
                });
                if(project[0]) {
                    project_no = project[0].project_no;
                    cabinet_name = project[0].cabinet_name;
                    append = '<span>'+project_no+' '+cabinet_name;
                    append += role == 'admin' ? '<span class="delete_employee" data-href="'+location.origin+'/designing_employees/'+ designing.id +'"><i class="far fa-trash-alt"></i></span><a class="edit_employee" href="'+location.origin+'/designing_employees/'+ designing.id+'/edit" rel="modal:open"><i class="fas fa-edit"></i></a>' : '';
                    append += '</span>'
                    $('td#'+user+'_'+designing.start_date).append(append);
                }
                /* $('td#'+user+'_'+designin.start_date).css('background',user_object.color) */
            });
        }
        /* $(this_row).remove(); */
...
function open_list () {
    ...
    $('.showCheckboxes').on('click',function(){
        ...
        $.each(users, function( index_user, user ) {
            ...
            checkbox_text += '<input name="user_id" type="radio" id="id_' + j + '_'+ i + '_'+ user.id + '" value="'+ user.id + '" ' +checked+' />';
