        ...
        <div style="width: 500px;max-width:100%;margin:auto;" id="mail_template">
			...
			<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" id="body" style="{!! $template_mail && $template_mail->mailStyle ? $template_mail->mailStyle->first()->style_body : '' !!}">
				@if( isset($text_body) && $text_body && count($text_body) > 0)
					...
					@endforeach
				@endif
				<p>{{ Sentinel::getUser()->first_name . ' ' . Sentinel::getUser()->last_name }} prijavljuje popravak sati </p>
				<p>za djelatnika {{ $workCorrecting->employee->user->first_name . ' ' .$workCorrecting->employee->user->last_name }}</p>
				<p>za datum {{ date("d.m.Y", strtotime($workCorrecting->date)) }}</p>
				<p>na projektu {{ $workCorrecting->project->erp_id . ' ' . $workCorrecting->project->name }}</p>
				<p>{{ $workCorrecting->comment }}</p>
					
				<form method="get" target="_blank" action="{{ route('confirmationWorkCorrecting') }}" style="overflow: hidden;">
					<input style="height: 34px;width: 100%; border-radius: 5px; border: 1px solid #ccc;" type="text" name="approved_reason" maxlength="191"><br>
					<input type="hidden" name="id" value="{{ $workCorrecting->id }}"><br>
					<div class="time">
						<label>Odobreno popravak sati:</label>
						<input name="approve_h" class="date form-control" type="time" value="{{ $workCorrecting->time }}" id="date1" required><i class="far fa-clock" style="border-radius: 5px; border: 1px solid #ccc"></i></i>
					</div>
				...
				<p><a href="{{ route('confirmationWorkCorrecting',['id'=> $workCorrecting->id,'approve'=> 1, 'approve_h'=> $workCorrecting->time, 'approved_reason'=>'']) }}">Potvrđeno</a> |
					<a href="{{ route('confirmationWorkCorrecting',['id'=> $workCorrecting->id,'approve'=> 0,'approve_h'=> '00:00',  'approved_reason'=>'']) }}">Odbijeno</a></p>
            </div>
