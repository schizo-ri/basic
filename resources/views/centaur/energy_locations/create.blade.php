<div class="modal-header">
	<h3 class="panel-title">Dodaj lokaciju</h3>
</div>
<div class="modal-body">
	<form accept-charset="UTF-8" role="form" method="post" action="{{ route('energy_locations.store') }}" enctype="multipart/form-data">
		<fieldset>
			...
			<div class="form-group {{ ($errors->has('address')) ? 'has-error' : '' }}">
				...
				<input class="form-control" name="address" type="text" maxlength="50" value="{{ old('address') }}" required />
				{!! ($errors->has('address') ? $errors->first('address', '<p class="text-danger">:message</p>') : '') !!}
			</div>
			<div class="form-group {{ ($errors->has('city')) ? 'has-error' : '' }}">
				<label for="">@lang('basic.city')</label>
				<input class="form-control" name="city" type="text" maxlength="20" value="{{ old('city') }}" required />
				{!! ($errors->has('city') ? $errors->first('city', '<p class="text-danger">:message</p>') : '') !!}
			</div>
			<div class="form-group {{ ($errors->has('phone')) ? 'has-error' : '' }}">
				<label for="">@lang('basic.phone')</label>
				<input class="form-control" placeholder="{{ __('basic.phone')}}" name="phone" maxlength="20" type="text" value="{{ old('phone') }}" required />
