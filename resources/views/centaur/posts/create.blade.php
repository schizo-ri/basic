    ...
    <div class="col-md-4 col-md-offset-4">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form accept-charset="UTF-8" role="form" method="post" action="{{ route('posts.store') }}" enctype="multipart/form-data">
					<fieldset>
						...
						<div class="form-group {{ ($errors->has('title')) ? 'has-error' : '' }}" >
							...
							<input class="form-control" placeholder="{{ __('basic.title')}}" name="title" type="text" value="{{ old('title') }}" required />
							{!! ($errors->has('title') ? $errors->first('title', '<p class="text-danger">:message</p>') : '') !!}
						</div>
						<div class="form-group {{ ($errors->has('content')) ? 'has-error' : '' }}">
							<label>@lang('basic.content')</label>
							<textarea name="content" type="text" class="form-control" rows="5" >{{ old('content') }}</textarea>
