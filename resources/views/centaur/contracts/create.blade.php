...
@section('title', __('basic.new_contract'))
		...
		<section>
			...
			<main class="all_documents">
				<div class="col-xs-12 offset-xs-0 col-sm-10 offset-sm-1 col-md-8 offset-md-2 main_create_contract">
					...
					<form class="form_contract" accept-charset="UTF-8" role="form" method="post" action="{{ route('contracts.store') }}">
						<fieldset>
								...
								<div class="form-group {{ ($errors->has('template_id')) ? 'has-error' : '' }}">
									...
									<select class="form-control" name="template_id" value="{{ old('template_id') }}" required>
										...
										@foreach ($templates as $template)
											<option value="{{ $template->id }}">Ugovor {{ $template->name }}</option>
										@endforeach	
									</select>
									...
									<div class="select_customer">
										<select class="form-control select_filter" name="customer_id" value="{{ old('customer_id') }}" id="customer_id" required>
											...
											@foreach ($customers as $customer)
												<option value="{{ $customer->id }}" {!! isset( $new_customer ) && $new_customer->id == $customer->id ? 'selected' : '' !!}>{{ $customer->name }}</option>
											@endforeach	
										</select>
									</div>
								...
								<div class="form-group datum {{ ($errors->has('date')) ? 'has-error' : '' }}">
									...
									<input class="form-control" name="date" type="date" maxlength="20" value="{{ old('date') }}" required />
								...
								<div class="form-group {{ ($errors->has('duration')) ? 'has-error' : '' }}">
									...
									<input class="form-control" name="duration" type="number" min="1" max="60" value="{{ old('duration') }}" required />
								...
								<div class="form-group {{ ($errors->has('invoice_no')) ? 'has-error' : '' }}">
									...
									<input class="form-control" name="invoice_no" maxlength="20"  type="text" value="{{ old('invoice_no') }}" required />
								...
								<div class="form-group datum {{ ($errors->has('invoice_date')) ? 'has-error' : '' }}">
									...
									<input class="form-control" name="invoice_date" type="date" maxlength="20" value="{{ old('invoice_date') }}" required />
								...
								<div class="test_group ">
									...
									<p class="contract_name"><span>c/b:</span><input class="form-control" name="package_prints_bw" type="number" min="0" value="{{ old('package_prints_bw') }}"  /></p>
									<p class="contract_name"><span>boja:</span><input class="form-control" name="package_prints_c" type="number" min="0" value="{{ old('package_prints_c') }}"  /></p>	
								</div>
								<p class="contract_title">Predmet ugovora</p>
								<div class="subject_group">
									<div class="subject">
										<p class="subject_no">Uređaj <span>1</span></p>
										<div class="form-group {{ ($errors->has('name')) ? 'has-error' : '' }}">
											<label >@lang('basic.subject_name')</label>
											<input class="form-control" name="subject_name[]" type="text" maxlength="100" value="{{ old('name') }}" required />
											{!! ($errors->has('name') ? $errors->first('name', '<p class="text-danger">:message</p>') : '') !!}
										</div>
										<div class="form-group {{ ($errors->has('serial_no')) ? 'has-error' : '' }}">
											<label >@lang('basic.serial_no')</label>
											<input class="form-control" name="subject_serial_no[]" type="text" maxlength="50" value="{{ old('serial_no') }}" required />
										...
										<div class="form-group ">
											...
											<p class="contract_name"><span>c/b:</span><input class="form-control" name="subject_counter_bw[]" type="number" min="0" value="{{ old('counter_bw') }}" required /></p>
											<p class="contract_name"><span>boja:</span><input class="form-control" name="subject_counter_c[]" type="number" min="0" value="{{ old('counter_c') }}" required /></p>			
										</div>
										<div class="form-group {{ ($errors->has('location_id')) ? 'has-error' : '' }}">
											<label>@lang('basic.subject_location')</label>
											<select class="form-control location_id" name="subject_location_id[]" value="{{ old('location_id') }}" required>
												<option value="" selected disabled></option>
											</select>
										...
										<div class="form-group">
											...
											<p class="contract_name"><input class="form-control price" name="subject_flat_rate[]" type="number" min="0" step="0.01" value="{{ old('flat_rate') }}" /><span class="float_right">Kn</span></p>
										</div>
										<div class="form-group ">
											<label >Ispisni paket uključen u mjesečni paušal</label>
											<p class="contract_name"><span>c/b:</span><input class="form-control" name="subject_no_prints_bw[]" type="number" min="0" value="{{ old('no_prints_bw') }}"  /></p>
											<p class="contract_name"><span>boja:</span><input class="form-control" name="subject_no_prints_c[]" type="number" min="0" value="{{ old('no_prints_c') }}"  /></p>	
										</div>
										<div class="form-group ">
											<label >Cijena otiska A4</label>
											<p class="contract_name"><span>c/b:</span><input class="form-control price_col" name="subject_price_a4_bw[]" type="number" step="0.01" min="0" value="{{ old('price_a4_bw') }}" required /><span class="float_right">Kn</span></p>
											<p class="contract_name"><span>boja:</span><input class="form-control price_col" name="subject_price_a4_c[]" type="number" step="0.01" min="0" value="{{ old('price_a4_c') }}" required /><span class="float_right">Kn</span></p>	
										</div>
									</div>
								</div>
