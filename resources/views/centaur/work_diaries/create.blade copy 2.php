	...
	<form class="form_work_diary" accept-charset="UTF-8" role="form" method="post" action="{{ route('work_diaries.store') }}">
		...
		@if(isset( $tasks ) && $tasks )
			<div class="form-group tasks {{ ($errors->has('erp_task_id')) ? 'has-error' : '' }}">
				...
				<select id="select-state" name="erp_task_id" placeholder="Izaberi zadatak..."  value="{{ old('erp_task_id') }}" id="sel1" required>
...
<script>
	...
	function getProjectEmpl ( employee_id, start_date ) {
		...
		$.ajax({
			...
			success: function( projects ) {
				...
				/* getTasks ( employee_id, start_date, Object.keys(projects)[0] ); */
	...
	function projectElement(projects) {
		var select_projects = '<div class="form-group select_project"><label>Projekt</label><select id="select_project" name="project_id" placeholder="Izaberi projekt..." id="sel1" required><option value="" disabled selected>Izaberi projekt</option>';
		$.each(projects, function( id, project ) {
			select_projects += '<option class="project_list" name="erp_task_id" value="'+id+'">'+project+'</option>';
		});
		select_projects += '</select></div>';
		$('.select_project').remove();
		if ($('form.form_work_diary').length > 0 ) {
			$('.form-group.datum').after(select_projects);
		}
	}
	
	function tastElement(tasks) {
		var select_tasks = '<div class="form-group tasks"><label>Zadatak</label><select id="select-state" name="erp_task_id" placeholder="Izaberi zadatak..." required><option value="" disabled selected>Izaberi zadatak</option>';
...
/* 	$.getScript('/../js/absence_create.js');
	if( $('select.form-control').length > 0 ) {
		$('select#select_project').select2({
			...
			dropdownParent: $('div.select_project'),
			width: 'resolve',
			placeholder: {
