...
<div class="modal-body">
	<form accept-charset="UTF-8" role="form" method="post" action="{{ route('employee_terminations.update', $employee_termination->id) }}" >
			...
			<div class="form-group {{ ($errors->has('termination_id')) ? 'has-error' : '' }}">
				...
				<select class="form-control" name="termination_id" value="{{ old('termination_id') }}" required >
					...
					@foreach ($terminations as $termination)
						<option name="termination_id" value="{{ $termination->id }}" {!! $employee_termination->termination_id == $termination->id ? 'selected' : '' !!} >{{ $termination->name }}</option>
					@endforeach	
				</select>
			...
			<div class="form-group datum date1 float_l  {{ ($errors->has('check_out_date')) ? 'has-error' : '' }}" >
				...
				<input name="check_out_date" id="date" class="form-control" type="date" value="{{ $employee_termination->check_out_date }}" required>
			...
			<div class="form-group clear_l  {{ ($errors->has('notice_period')) ? 'has-error' : '' }}" >
				...
				<input name="notice_period" id="date" class="form-control" type="text" value="{{ $employee_termination->notice_period }}" maxlength="100" required>
			...
			<div class="form-group clear_l {{ ($errors->has('comment')) ? 'has-error' : '' }}">
				...
				<textarea rows="4" name="comment" type="text" class="form-control" value="" maxlength="16535" required>{{ $employee_termination->comment }}</textarea>
