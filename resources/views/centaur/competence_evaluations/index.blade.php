								...
								<div class="div_select2">
									<select name="employee_id" class="select_filter filter_evaluation " style="border: none;font-size: inherit;color: inherit;" required >
										...
										@foreach ( $employees as $employee )
											<option value="{!! $employee->user->last_name  . ' ' . $employee->user->first_name !!}">{!! $employee->user->last_name  . ' ' . $employee->user->first_name !!}</option>
										@endforeach
									</select>
								</div>
							@endif
							...
							<table id="index_table" class="display table table-hover sort_1_asc">
								...
								<tbody>
									@foreach ($competence->hasGroups as $key_group => $group)
										<tr>
											<td class="td_group" colspan="6">{{ $group->name }}</td>										
										</tr>
										@foreach ($group->hasQuestions as $key_question => $question)
											<tr>
												<td class="td_question" colspan="6">{{ $question->name }}</td>
											</tr>
											@foreach ($question->hasEvaluations->groupBy('employee_id') as $employee_id => $evaluation)
												...
												{{-- @if ($evaluation_empl)
													<tr class="tr_evaluation">
														<td class="td_evaluation">{{ $employee->user->last_name . ' ' . $employee->user->first_name }}</td>
														<td class="td_evaluation">{!! $evaluation_empl->rating->rating !!}</td>
														<td class="td_evaluation rating_empl">{!! $evaluation_empl->rating->rating * $evaluation_empl->question->rating * $evaluation_empl->question->group->coefficient !!}</td>
														<td class="td_evaluation">{{ $evaluation_empl->rating->description }}</td>
														<td class="td_evaluation edit_evaluation_id editable">
															@if( count($competence->hasRatings) > 0 )
																@foreach ($competence->hasRatings as $key_rating => $rating)
																	<div class="rating_radio evaluate_manager">
																		<input 
																			type="radio" 
																			name="rating_id[{{$evaluation_empl->id}}]"
																			value="{{ $rating->id }}" 
																			title="{{ $evaluation_empl->id }}" id="id[{{$evaluation_empl->id}}][{{$key_rating}}]"
																			{!! $evaluation_admin->where('rating_id', $rating->id )->first() ? 'checked' : '' !!}>
																		<label for="id[{{$evaluation_empl->id}}][{{$key_rating}}]">{{ $rating->rating }}</label>
																	</div>
																@endforeach
															@endif
														</td>
														<td class="td_evaluation edit_evaluation_comment editable">
															<textarea class="form-control evaluation_comment" name="comment" rows="3" title="{{ $evaluation_empl->id }}" >{!! $evaluation_admin->first() ? $evaluation_admin->first()->comment : '' !!}</textarea>
														</td>
													</tr>
												@endif --}}
											@endforeach
