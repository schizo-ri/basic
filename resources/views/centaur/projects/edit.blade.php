			...
			<div class="form-group {{ ($errors->has('employee_id')) ? 'has-error' : '' }}">
				...
				<select class="form-control" name="employee_id" >
					...
					@foreach ($employees as $employee)
						@if ($employee->user)
							<option name="employee_id" value="{{ $employee->id }}" {!!  $project->employee_id != null && $project->employee_id == $employee->id ? 'selected' : '' !!} >{{ $employee->user->first_name . ' ' . $employee->user->last_name }}</option>
						@endif
					@endforeach	
				</select>
