    ...
    <div class="col-md-4 col-md-offset-4">
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form accept-charset="UTF-8" role="form" method="post" action="{{ route('evaluation_questions.update', $evaluationQuestion->id) }}">
					<div class="form-group {{ ($errors->has('category_id'))  ? 'has-error' : '' }}">
						...
						<select  class="form-control"  name="category_id" value="{{ old('category_id') }}" autofocus required >
							...
							@foreach ($categories as $category)
								<option value="{{ $category->id }}" {!! $evaluationQuestion->id == $category->id ? 'selected' : '' !!}>{{ $category->name }}</option>
							@endforeach
						</select>
					...
					<div class="form-group {{ ($errors->has('name'))  ? 'has-error' : '' }}">
						...
						<input name="name" type="text" class="form-control" value="{{ $evaluationQuestion->name }}"  required >
						{!! ($errors->has('name') ? $errors->first('name', '<p class="text-danger">:message</p>') : '') !!}
					</div>
					<div class="form-group {{ ($errors->has('description'))  ? 'has-error' : '' }}">
						<label>@lang('basic.description')</label>
						<textarea  name="description" type="text" class="form-control" rows="3" required >{{ $evaluationQuestion->description }}</textarea>
