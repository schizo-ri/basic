...
<div class="modal-body">
	<form accept-charset="UTF-8" role="form" method="post" action="{{ route('travel_orders.store') }}">
		...
		<div class="form-group {{ ($errors->has('days')) ? 'has-error' : '' }}">
			...
			<input name="days" type="number" class="form-control" value="{!! old('days') ? old('days') : 1 !!}" required>
		...
		<div class="form-group datum {{ ($errors->has('start_date')) ? 'has-error' : '' }}">
			...
			<input name="start_date" type="datetime-local" class="form-control" value="{!! old('start_date') ? old('start_date') : Carbon\Carbon::now()->format('Y-m-d\TH:i') !!}" required>
		...
		<div class="form-group datum {{ ($errors->has('end_date')) ? 'has-error' : '' }}">
			...
			<input name="end_date" type="datetime-local" class="form-control" value="{!! old('end_date') ? old('end_date') : Carbon\Carbon::now()->format('Y-m-d\TH:i') !!}" required>
		...
		@if(Sentinel::getUser()->hasAccess(['travel_orders.update']))
			<div class="form-group {{ ($errors->has('calculate_employee')) ? 'has-error' : '' }}">
				...
				<select class="form-control" name="calculate_employee" value="{{ old('calculate_employee') }}" >
					<option selected disabled></option>
					@foreach ($employees as $employee)
						<option value="{{ $employee->id }}">{{ $employee->user['first_name'] . ' ' .  $employee->user['last_name'] }}</option>
					@endforeach
			...
			<div class="form-group {{ ($errors->has('advance')) ? 'has-error' : '' }}">
				...
				<input name="advance" type="number" step="0.01" class="form-control" value="{!! old('advance') ? old('advance') : 0.00 !!}" >
				{!! ($errors->has('days') ? $errors->first('days', '<p class="text-danger">:message</p>') : '') !!}
			</div>
			<div class="form-group datum {{ ($errors->has('advance_date')) ? 'has-error' : '' }}">
				<label>@lang('basic.advance_date')</label>
				<input name="advance_date" type="date" class="form-control" value="{{ old('advance_date') }}" >
			...
			<div class="form-group {{ ($errors->has('rest_payout')) ? 'has-error' : '' }}">
				...
				<input name="rest_payout" type="number" step="0.01" class="form-control" value="{!! old('rest_payout') ? old('rest_payout') : 0.00 !!}"" >
