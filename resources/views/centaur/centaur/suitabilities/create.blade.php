...
<div class="modal-body">
	<form class="form_benefit" accept-charset="UTF-8" role="form" method="post" action="{{ route('suitabilities.store' ) }}" enctype="multipart/form-data">
		<fieldset>
			...
			<div class="form-group {{ ($errors->has('contact'))  ? 'has-error' : '' }}">
				...
				<input name="contact" type="text" class="form-control" maxlength="100" value="{{ old('contact') }}"  >
...
<script>
	...
	$('#file').change(function(){
        $('#file_name').text( $('input[name=fileToUpload]').val());
	});
	$('#doc').change(function(){
        $('#doc_name').text( $('input[name=docToUpload]').val());
	});
</script>
