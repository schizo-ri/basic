	...
	<main class="col-md-12 index_main main_documents float_right">
		<section>
			<div class="page-header header_document">
				...
				@lang('basic.education') {{ $education->name }}
			</div>
					...
					<header class="page-header">
						<div class="index_table_filter">
							...
							@if(Sentinel::getUser()->hasAccess(["education_themes.create"]) || in_array("education_themes.create", $permission_dep) )
								<a class="add_new" href="{{ route('education_themes.create',['education_id'=>$education->id ]) }}" rel="modal:open" title="{{ __('basic.add_educationTheme') }}">
					...
					<section class="page-main">
						@if( count( $education->educationThemes )>0 )
							<table id="index_table" class="display table table-hover">
								...
								<tbody>
									@foreach ( $education->educationThemes as $educationTheme)
										<tr class="tr_open_link_new_page"  data-href="/education_themes/{{ $educationTheme->id }}" >
											<td>{{ $educationTheme->name }}</td>
											@if (Sentinel::inRole('administrator'))
