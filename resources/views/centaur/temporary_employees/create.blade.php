...
<div class="modal-body">
	<form accept-charset="UTF-8" role="form" method="post" action="{{ route('temporary_employees.store') }}">
		<fieldset>
			<div class="form-group {{ ($errors->has('user_id')) ? 'has-error' : '' }}">
				...
				<select class="form-control" name="user_id" required>
					...
					@foreach($users as $user)					
						@if( ! Sentinel::findById($user->id)->inRole('superadmin') && ! $employees->where('user_id', $user->id)->first() && ! $temporary_employees->where('user_id', $user->id)->first() )
