...
<div class="modal-body">
	<form accept-charset="UTF-8" role="form" method="post" action="{{ route('vacations.update', $vacation->id) }}">
		...
		<div class="form-group {{ ($errors->has('no_week'))  ? 'has-error' : '' }} clear_l" id="period">
			...
			<input name="no_week" type="number" class="form-control" min="1" max="4" value="{{  $vacation->no_week }}" required>
		</div>
		<div class="form-group">
			<label>Status</label>
			<input type="radio" id="status_0" name="active" value="0" {!! $vacation->active == 0 ? 'checked' : '' !!} /><label for="status_0">@lang('basic.inactive')</label>
			<input type="radio" id="status_1" name="active" value="1" {!! $vacation->active == 1 ? 'checked' : '' !!} /><label for="status_1">@lang('basic.active')</label>
		</div>
		<div class="form-group">
			<table>
				<thead>
								...
								@foreach ($department->hasEmployeeDepartment as $employeeDepartment)
									@if ( ! $employeeDepartment->employee->checkout )
										<span class="col-6 float_l">
											<input type="checkbox" name="employee_id[{{ $department->id}}][]" id="employee_id[{{ $department->id}}][{{$i}}]" value="{{ $employeeDepartment->employee_id }}" 						{!! $dep_plan && isset($dep_plan['employees'] ) && in_array( $employeeDepartment->employee_id, explode( ',', $dep_plan['employees'])) ? 'checked' : '' !!}
											
											/>
											<label for="employee_id[{{ $department->id}}][{{$i}}]">{{ $employeeDepartment->employee->user['last_name']  . ' ' . $employeeDepartment->employee->user['first_name'] }} </label>
										</span>
										@php
											$i++;
										@endphp
