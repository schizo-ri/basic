...
<div class="modal-body">
	<form accept-charset="UTF-8" role="form" method="post" action="{{ route('vacations.store') }}">
		...
		<div class="form-group {{ ($errors->has('no_week'))  ? 'has-error' : '' }} clear_l" id="period">
			...
			<input name="no_week" type="number" class="form-control" min="1" max="21" value="{!! old('no_week') ? old('no_week') : 1 !!}" required>
		</div>
		<div class="form-group">
			<label>Status</label>
			<input type="radio" id="status_0" name="active" value="0" checked /><label for="status_0"  class="decree">@lang('basic.inactive')</label>
			<input type="radio" id="status_1" name="active" value="1" /><label for="status_1"  class="decree">@lang('basic.active')</label>
		</div>
		<div class="form-group">
			<table>
				<thead>
										...
										@foreach ($works->workers as $employee)
											@if ( ! $employee->checkout )
												<span class="col-6 float_l">
													<input type="checkbox" name="employee_id[{{ $department->id}}][]" id="employee_id[{{ $department->id}}][{{$i}}]" value="{{ $employee->id }}" /><label class="decree" for="employee_id[{{ $department->id}}][{{$i}}]">{{ $employee->user['last_name']  . ' ' . $employee->user['first_name'] }} </label>
												</span>
												@php
													$i++;
												@endphp
