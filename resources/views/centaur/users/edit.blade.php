...
<div class="modal-header">
    <h3 class="panel-title">@lang('ctrl.password_edit')</h3>
</div>
<div class="modal-body">
    <form accept-charset="UTF-8" class="form_edit_user form_user" role="form" method="post" action="{{ route('users.update', $user->id) }}" enctype="multipart/form-data" >
        @if ( ! strpos($_SERVER['REQUEST_URI'],'edit_user'))
            ...
            <div class="second_tab">
                <div class="form-group  {{ ($errors->has('password')) ? 'has-error' : '' }}">
                    ...
                    <input class="form-control" maxlength="191" name="password" id="password" type="password" >
                  </div>
        ...
        @else
            <div class="form-group user_name {{ ($errors->has('first_name')) ? 'has-error' : '' }}">
                <input class="form-control" id="first_name" name="first_name" maxlength="191" type="hidden" value="{{ $user->first_name }}"  />
                {!! ($errors->has('first_name') ? $errors->first('first_name', '<p class="text-danger">:message</p>') : '') !!}
            </div>
            <div class="form-group user_name {{ ($errors->has('last_name')) ? 'has-error' : '' }}">
                <input class="form-control" name="last_name" maxlength="191" id="last_name" type="hidden" value="{{ $user->last_name}}"  />
                {!! ($errors->has('last_name') ? $errors->first('last_name', '<p class="text-danger">:message</p>') : '') !!}
            </div>
            <div class="form-group email {{ ($errors->has('email')) ? 'has-error' : '' }}">
                <input class="form-control" name="email" id="email" type="hidden" maxlength="191" value="{{ $user->email}}"  >
                {!! ($errors->has('email') ? $errors->first('email', '<p class="text-danger">:message</p>') : '') !!}
            </div>
            <div class="form-group  {{ ($errors->has('password')) ? 'has-error' : '' }}">
                <label>@lang('basic.password')</label>
