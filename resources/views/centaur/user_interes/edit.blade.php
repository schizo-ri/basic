...
<div class="modal-body">
    <form accept-charset="UTF-8" role="form" method="post" action="{{ route('user_interes.update', $userInteres->id) }}">
        @if($tag)
            <div class="form-group {{ ($errors->has('category')) ? 'has-error' : '' }}">
                <label>@lang('basic.interest')</label>
                <input name="category" type="text" class="form-control" maxlength="191" value="{{ $userInteres->category }}" />
        ...
        @else
            <div class="form-group description {{ ($errors->has('description')) ? 'has-error' : '' }}">
                ...
                <textarea name="description" class="form-control" type="text" >{{ $userInteres->description }}</textarea>
