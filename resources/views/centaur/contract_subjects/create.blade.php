...
<div class="modal-body">
	<form accept-charset="UTF-8" role="form" method="post" action="{{ route('contract_subjects.store') }}">
		<fieldset>
			<div class="form-group {{ ($errors->has('contract_id')) ? 'has-error' : '' }}">
				...
				<select class="form-control" name="contract_id" value="{{ old('contract_id') }}" >
					...
					@foreach ($contracts as $contract)
						<option value="{{ $contract->id }}" {!! $contract_id && $contract_id == $contract->id ? 'selected' : '' !!}>Ugovor {{ $contract->template->name }}</option>
					@endforeach	
				</select>
			...
			<div class="form-group {{ ($errors->has('location_id')) ? 'has-error' : '' }}">
				...
				<select class="form-control" name="location_id" value="{{ old('location_id') }}" >
					...
					@foreach ($customer_locations as $customer_location)
						<option value="{{ $customer_location->id }}">{{ $customer_location->customer->name . ' - ' . $customer_location->address . ', ' . $customer_location->city  }}</option>
					@endforeach	
				</select>
			...
			<div class="form-group {{ ($errors->has('name')) ? 'has-error' : '' }}">
				...
				<input class="form-control" name="name" type="text" maxlength="100" value="{{ old('name') }}" required />
				{!! ($errors->has('name') ? $errors->first('name', '<p class="text-danger">:message</p>') : '') !!}
			</div>
			<div class="form-group {{ ($errors->has('serial_no')) ? 'has-error' : '' }}">
				<label >@lang('basic.serial_no')</label>
				<input class="form-control" name="serial_no" type="text" maxlength="50" value="{{ old('serial_no') }}" required />
			...
			<div class="form-group ">
				...
				<p class="contract_name"><span>c/b:</span><input class="form-control" name="counter_bw" type="number" min="0" value="{{ old('counter_bw') }}" required /></p>
				<p class="contract_name"><span>boja:</span><input class="form-control" name="counter_c" type="number" min="0" value="{{ old('counter_c') }}" required /></p>			
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<p class="contract_name"><input class="form-control price" name="flat_rate" type="number" min="0" step="0.01" value="{{ old('flat_rate') }}" /><span class="float_right">Kn</span></p>
			</div>
			<div class="form-group ">
				<label >Ispisni paket uključen u mjesečni paušal</label>
				<p class="contract_name"><span>c/b:</span><input class="form-control" name="no_prints_bw" type="number" min="0" value="{{ old('no_prints_bw') }}"  /></p>
				<p class="contract_name"><span>boja:</span><input class="form-control" name="no_prints_c" type="number" min="0" value="{{ old('no_prints_c') }}"  /></p>	
			</div>
			<div class="form-group ">
				<label >Cijena otiska A4</label>
				<p class="contract_name"><span>c/b:</span><input class="form-control price_col" name="price_a4_bw" type="number" step="0.01" min="0" value="{{ old('price_a4_bw') }}" required /><span class="float_right">Kn</span></p>
				<p class="contract_name"><span>boja:</span><input class="form-control price_col" name="price_a4_c" type="number" step="0.01" min="0" value="{{ old('price_a4_c') }}" required /><span class="float_right">Kn</span></p>	
			</div>
			<div class="form-group ">
				<label >Jamstvo</label>
