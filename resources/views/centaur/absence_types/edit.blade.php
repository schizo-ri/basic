...
<div class="modal-body">
	<form class="absence" accept-charset="UTF-8" role="form" method="post" action="{{ route('absence_types.update', $absenceType->id) }}">
		...
		<div class="form-group {{ ($errors->has('erp_id'))  ? 'has-error' : '' }}">
			<label>ERP ID</label>
			<input name="erp_id" type="text" class="form-control" maxlength="10" value="{{ $absenceType->erp_id }}"  >
		...
		<div class="form-group {{ ($errors->has('active'))  ? 'has-error' : '' }}">
			...
			<input name="active" type="checkbox" value="1" {!! $absenceType->active == 1 ? 'checked' : '' !!}>
